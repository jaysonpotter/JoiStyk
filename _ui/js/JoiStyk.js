window.onload = function () {
	...
	var padRad = 80;
	var stickRad = 40;
	var trackRad = 40; // <- this will need to go
	var followSpeed = 1;
	...
	var drawFollowDot = function() {
		ctx.beginPath();
		ctx.arc(trackPos.x, trackPos.y, trackRad, 0, 2 * Math.PI, false);
		ctx.closePath();
	...
	function draw() {
		...
		if(padPos && stickPos && trackPos){
			...
			// This is going to move the pad around in case the user has gone really far out of the pad area, it also prevents the stick from some odd behavior. Try commenting out the stickPos updates in this minirithm
			if((trackPos.x - stickRad) < (padPos.x - padRad)){
				padPos.x = (padPos.x - followSpeed);
